...
const { inspector, getCapitalizeStr, objectHash } = require('../lib');
...
module.exports = async function servicesConstraint(context) {
  ...
  switch (`${hookHelper.contextPath}.${hookHelper.contextMethod}.${hookHelper.contextType}`) {
    ...
    case 'opcua-values.create.after':
      validate = async (record) => {
        if (isDebug && record) inspector(`hook.opcua-values.create.after('${record.tagName}'):`, record);
        if (record.tagId) {
          tagId = record.tagId.toString();
